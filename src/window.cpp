...
static inline qreal sign(qreal v)
{
    return (v > 0) - (v < 0);
}
...
#else
            debug << ", windowId=0x" << hex << window->window() << dec;
#endif
        }
...
void Window::handleInteractiveMoveResize(const QPointF &local, const QPointF &global)
{
    ...
    if (!isRequestedFullScreen() && isInteractiveMove()) {
        if (quickTileMode() != QuickTileMode(QuickTileFlag::None)
            ...
            GeometryUpdatesBlocker blocker(this);
            setQuickTileMode(QuickTileFlag::None);
            QRectF geom_restore = geometryRestore();
            if (!geom_restore.isValid()) {
                // When the window has been mapped maximized, the geometryRestore is not valid.
                // Although configure(0,0) will be sent to the client and the client will set a
                // reasonable geometry in the next state when the window has an invalid move resize geometry,
                // Update moveResizeGeometry according to the committed state is not allowed in interactive move/resize.
                // MoveResizeGeometry will keep invalid in interactiveMoveResize, which will influence most
                // of the calculations in handleInteractiveMoveResize. E.g. the calculation of the exposed pixels
                // will always be zero, causing move failure.
                // We must set a reasonable and valid moveResizeGeometry in every frame of interactiveMoveResize.
                // Here we generate a reasonable geometryRestore for such case.
                // TODO: Firstly unmaximize the window before interactiveMoveResize to avoid such problem.
                QRectF clientArea;
                if (isElectricBorderMaximizing()) {
                ...
                geom_restore.moveTo(clientArea.topLeft());
                geom_restore.setSize(constrainFrameSize(QSize(width() * 2 / 3, height() * 2 / 3), SizeModeMax));
        ...
        if (!isMovable()) { // isMovableAcrossScreens() must have been true to get here
            ...
            if (!isUnrestrictedInteractiveMoveResize()) {
                ...
                for (;;) {
                    ...
                    qreal xRawDelta = currentMoveResizeGeom.x() - currentTry.x(),
                          yRawDelta = currentMoveResizeGeom.y() - currentTry.y();
                    qreal xSign = sign(xRawDelta),
                          ySign = sign(yRawDelta);
                    qreal dx = xSign * qMin(qAbs(xRawDelta), 1.0),
                          dy = ySign * qMin(qAbs(yRawDelta), 1.0);
...
void Window::moveResize(const QRectF &rect, bool immediateResize)
{
    setMoveResizeGeometry(rect);
    if (s_placeholderWindow && s_placeholderWindow->takeOver(window())) {
        s_placeholderWindow->setGeometry(rect.toRect());
    ...
    moveResizeInternal(rect, MoveResizeMode::MoveResize, immediateResize);
}
...
void Window::checkWorkspacePosition(QRectF oldGeometry, const VirtualDesktop *oldDesktop)
{
    ...
    if (isRequestedFullScreen() || requestedMaximizeMode() != MaximizeRestore || quickTileMode() != QuickTileMode(QuickTileFlag::None)) {
        moveResize(ensureSpecialStateGeometry(newGeom), true);
        m_fullscreenGeometryRestore = moveToArea(m_fullscreenGeometryRestore, oldScreenArea, screenArea);
        m_maximizeGeometryRestore = moveToArea(m_maximizeGeometryRestore, oldScreenArea, screenArea);
        return;
    }
