...
public:
    ...
    void _d_onPropertyChanged(xcb_atom_t atom)
    {
...
#endif
    {
        ...
        if (response_type == XCB_PROPERTY_NOTIFY) {
            ...
            if (Q_LIKELY(monitorRootWindowProperty)) {
                static xcb_window_t root = static_cast<xcb_window_t>(QX11Info::appRootWindow());
...
bool KWinUtils::isSupportedAtom(quint32 atom) const
{
    ...
    const xcb_atom_t *atoms = reinterpret_cast<const xcb_atom_t *>(static_cast<const void *>(datas.constData()));
...
QVariant KWinUtils::getGtkFrame(const QObject *window) const
{
    ...
    const int32_t *datas = reinterpret_cast<const int32_t *>(static_cast<const void *>(data.constData())); // left right top bottom
...
bool KWinUtils::isDeepinOverride(const QObject *window) const
{
    ...
    return *reinterpret_cast<const int32_t *>(static_cast<const void *>(data.constData())) == 1;
