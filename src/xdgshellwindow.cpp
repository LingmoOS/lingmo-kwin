...
void XdgSurfaceWindow::moveResizeInternal(const QRectF &rect, MoveResizeMode mode, bool immediateResize)
{
    ...
    if (mode != MoveResizeMode::Move) {
        ...
        if (size() != rect.size()) {
            if (qobject_cast<const XdgToplevelWindow *>(this)) {
                return;
            }
        }
...
void XdgSurfaceWindow::installPlasmaShellSurface(PlasmaShellSurfaceInterface *shellSurface)
{
    ...
    auto updatePosition = [this, shellSurface] {
        if (pos() == QPointF(shellSurface->position())) {
            return;
        }
        move(shellSurface->position());
        if (shellSurface->position().x() < 0 || shellSurface->position().y() < 0) {
