    ...
    , m_platform(backend)
{
    ...
    qCInfo(KWIN_DRM) << "Find gpu:" << version->name;
...
bool DrmGpu::updateOutputs()
{
    ...
    for (auto it = m_connectors.begin(); it != m_connectors.end();) {
        ...
        qCInfo(KWIN_DRM) << "Update outputs: on GPU" << m_devNode << " model:" << conn->modelName()
                         << "\nname:" << conn->connectorName() << " Connection:" << conn->isConnected() << " StillExists:" << stillExists;
        if (!stillExists || !conn->isConnected()) {
            ...
            qCInfo(KWIN_DRM) << "New" << (conn->isNonDesktop() ? "non-desktop " : "")
                             << "output on GPU:" << m_devNode << " name:" << output->name();
            m_drmOutputs << output;
            addedOutputs << output;
...
void DrmGpu::updateCursorSize(const QSize &size)
{
    m_cursorSize = m_gpuCursorSize;
    if (size.width() < m_gpuCursorSize.width())
        m_cursorSize.setWidth(size.width());

    if (size.height() < m_gpuCursorSize.height())
        m_cursorSize.setHeight(size.height());
}
